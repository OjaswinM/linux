...
static void test_extent_split(struct kunit *test)
{
	struct ext4_ext_path *path;
...
static const struct kunit_ext_test_param ext_test_params[] = {
...
static int ext_param_init(struct kunit *test)
{
	size_t arr_size = ARRAY_SIZE(ext_test_params);

	kunit_register_params_array(test, ext_test_params, arr_size,
				    ext_get_desc);
	return 0;
}
 ...
 */
static struct kunit_case extents_test_cases2[] = {
	KUNIT_CASE_PARAM_WITH_INIT(test_extent_split, kunit_array_gen_params,
				   ext_param_init, NULL),
	{}
};
...
static struct kunit_suite extents_test_suite = {
	...
	.test_cases = extents_test_cases2,
};
